...
namespace FITSKIP.Application.Services
{
    public class MaintenanceWorkOrderService : IMaintenanceWorkOrderService
    {
        ...
        public async Task<MaintenanceWorkOrderDTO> CreateWorkOrderAsync(CreateMaintenanceWorkOrderRequest request, string userId)
        {
            ...
            else if (pendingOrOverdueWO != null && pendingOrOverdueWO.Status == "Quá hạn")
            {
        ...
        private async Task ValidateScheduledDateAsync(MaintenanceWorkOrder workOrder, DateTime scheduledDate, string? assignedElectrical, string? assignedMechanical)
        {
            ...
            if (isPostponed && workOrder.PostponedDueDate.HasValue)
            {
                if (scheduledDateOnly > workOrder.PostponedDueDate.Value.Date)
                {
                    throw new InvalidOperationException(
                        $"❌ Không thể giao việc sau ngày hoãn ({workOrder.PostponedDueDate.Value:dd/MM/yyyy}). " +
        ...
        public async Task<MaintenanceWorkOrderDTO> StartWorkOrderAsync(int workOrderId, string technicianId)
        {
            ...
            else if (today < workOrder.ScheduledDate.Date)
            {
                throw new InvalidOperationException(
        ...
        public async Task<MaintenanceWorkOrderDTO> CompleteWorkOrderAsync(int workOrderId, CompleteWorkOrderRequest request, string technicianId)
        {
            ...
            if (workOrder.Status != "Quá hạn" && today < workOrder.ScheduledDate.Date)
            {
                throw new InvalidOperationException(
                    $"❌ Chưa thể thực hiện bảo trì. Ngày dự định bảo trì là {workOrder.ScheduledDate:dd/MM/yyyy}. " +
                    $"Vui lòng chờ đến đúng ngày."
                );
            }
        ...
        public async Task<MaintenanceChecklistItemDTO> UpdateChecklistItemAsync(int checklistId, UpdateChecklistItemRequest request, string technicianId)
        {
            ...
            // ✅ VALIDATE: Lấy WorkOrder để kiểm tra ngày và status
            var workOrder = await _workOrderRepository.GetByIdAsync(item.WorkOrderId);
            if (workOrder == null)
                throw new InvalidOperationException($"Work order not found: {item.WorkOrderId}");

            // ✅ VALIDATE: Kiểm tra ngày trước khi cho phép tick checklist
            var today = DateTime.Today;

            if (workOrder.Status == "Quá hạn")
            {
                // Quá hạn: Cho phép làm bất cứ lúc nào
            }
            else if (workOrder.Status == "Hoãn")
            {
                if (today < workOrder.ScheduledDate.Date)
                {
                    throw new InvalidOperationException(
                        $"❌ Chưa thể thực hiện bảo trì. Phiếu đã được hoãn đến ngày {workOrder.ScheduledDate:dd/MM/yyyy}. " +
                        $"Vui lòng chờ đến đúng ngày để thực hiện."
                    );
                }
            }
            else if (today < workOrder.ScheduledDate.Date)
