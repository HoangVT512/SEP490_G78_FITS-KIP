...
const IncidentList = () => {
  ...
  useEffect(() => {
    const handleDataUpdated = (data) => {
      console.log("🔄 [IncidentList] Received data update:", data);
      if (data && data.type === "incident") {
        console.log("✅ [IncidentList] Refreshing incidents...");
        fetchIncidents(false); // Refresh without loading spinner
      }
    };

    signalRService.onDataUpdated(handleDataUpdated);

    return () => {
      signalRService.offDataUpdated(handleDataUpdated);
    };
  }, []);
