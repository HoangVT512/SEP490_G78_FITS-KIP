...
const MaintenanceReports = () => {
    ...
    try {
      ...
      if (response?.success && response?.data) {
        ...
        console.log("Unique status values:", [
          ...new Set(allData.map((wo) => wo.status)),
        ]);
        ...
        if (statusFilter !== "all") {
          filteredData = allData.filter((wo) => {
            ...
            console.log(
              `Checking status: ${status} vs filter: ${statusFilter}`
            );
            ...
            return (
              ...
              (statusFilter === "InProgress" &&
                (status === "Đang thực hiện" || status === "InProgress")) ||
              (statusFilter === "Completed" &&
                (status === "Hoàn thành" || status === "Completed")) ||
              (statusFilter === "Closed" &&
                (status === "Đã đóng" || status === "Closed"))
            );
          });
        }

        console.log(
          `Filtered ${filteredData.length} items from ${allData.length} total`
        );
  ...
  const calculateStatistics = (data) => {
    ...
    const efficiency =
      data.length > 0
        ? Math.round(((completed + closed) / data.length) * 100)
        : 0;
      ...
      render: (_, record) => (
        <strong>
          ...
          {record.workOrderCode ||
            `WO${String(record.workOrderId || 0).padStart(3, "0")}`}
        </strong>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const electricalTech =
          record.assignedToElectricalName ||
          record.electricalTechnicianName ||
          record.electricalTechnician ||
          record.assignedToElectrical;
        const mechanicalTech =
          record.assignedToMechanicalName ||
          record.mechanicalTechnicianName ||
          record.mechanicalTechnician ||
          record.assignedToMechanical;
        ...
        return (
          <Space size="small">
            ...
            {(!electricalTech || electricalTech === "-") &&
              (!mechanicalTech || mechanicalTech === "-") && (
    ...
    >
      <Spin spinning={loading}>
          ...
          >
            <div>
              ...
              <p style={{ margin: "8px 0 0 0", color: "#8c8c8c" }}>
                Tổng quan quản lý bảo trì thiết bị -{" "}
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
          <Col flex="1 1 20%">
            ...
            >
              <Statistic
                ...
                prefix={
                  <ToolOutlined style={{ color: "black", marginRight: 8 }} />
                }
          ...
          <Col flex="1 1 20%">
            ...
            >
              <Statistic
                title={
                  <span style={{ color: "rgba(0, 0, 0, 0.85)" }}>Đã đóng</span>
                }
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
          <Col xs={24} sm={12}>
            ...
            >
              ...
              <Progress
                percent={
                  statistics.total > 0
                    ? ((statistics.overdue || 0) / statistics.total) * 100
                    : 0
                }
                strokeColor="#ff4d4f"
          ...
          <Col xs={24} sm={12}>
            <Card
              title={
                <>
                  <CheckCircleOutlined /> Hiệu Suất Hoàn Thành
                </>
              }
              bordered={false}
              style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
        ...
        <Card
          title={
            <>
              <TeamOutlined /> Danh Sách Phiếu Bảo Trì
            </>
          }
          bordered={false}
          style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
        >
          <Table
            ...
            rowKey={(record) =>
              record.workOrderId || record.id || record.planId
            }
            pagination={{
              pageSize: 10,
