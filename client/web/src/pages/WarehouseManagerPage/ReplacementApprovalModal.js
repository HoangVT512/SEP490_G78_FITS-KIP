...
const ReplacementApprovalModal = ({
  ...
  useEffect(() => {
    ...
    load();
    fetchUsers();
  }, [open, incidentId, workOrderId]); // ✅ THÊM workOrderId vào dependencies

  const load = async () => {
    if (viewMode) {
      // Khi viewMode=true (xem lịch sử)
      // Ưu tiên load theo incidentId hoặc workOrderId, nếu không có thì load theo equipmentId
      const loadId = incidentId || workOrderId || equipmentId;
      const isLoadingByIncident = !!incidentId;
      const isLoadingByWorkOrder = !!workOrderId;
      ...
      try {
        ...
        } else if (isLoadingByWorkOrder) {
          // ✅ Load theo workOrderId
          res = await replacementHistoryService.getByWorkOrderId(workOrderId);
          data = Array.isArray(res) ? res : res?.data || [];
        } else {
        ...
        const approved = data
          .filter((r) => r.status === "Hoàn thành" || r.status === "Completed")
          .sort((a, b) => {
        ...
        message.error(
          "Không thể tải lịch sử thay thế: " + (err?.message || err)
        );
      } finally {
        setLoading(false);
      }
    } else {
      // Logic cho chế độ duyệt yêu cầu (không phải viewMode)
      // ✅ Hỗ trợ cả incidentId và workOrderId
      ...
      try {
        ...
        message.error(
          "Không thể tải yêu cầu thay thế: " + (err?.message || err)
        );
      } finally {
        setLoading(false);
      }
  ...
  const fetchSparePartsStock = async (items) => {
    try {
      const sparePartIds = [
        ...new Set(items.map((item) => item.sparePartId).filter(Boolean)),
      ];
      const stockData = {};
    ...
    try {
      ...
      if (newStatus === "Đã duyệt cấp phát") {
        try {
          ...
          console.log(
            `Checking inventory for Part ID: ${partId}, Requested: ${requestedQuantity}`
          );
          ...
          if (!sparePart) {
            message.error(
              "❌ Không tìm thấy thông tin phụ tùng trong hệ thống"
            );
            setLoading(false);
            return;
          }
          ...
          const partNumber =
            sparePart.partNumber || sparePart.PartNumber || "N/A";

          console.log(
            `Available in stock: ${availableQuantity}, Part: ${partNumber} - ${partName}`
          );
          ...
          console.log(
            `✅ Stock is sufficient. Available: ${availableQuantity}, Requested: ${requestedQuantity}`
          );
          // message.info({
          //   content: (
        ...
        if (newStatus === "Đã duyệt cấp phát") {
          try {
            ...
            await notificationService.sendToTechnicians({
              message: `Linh kiện ${
                record.partName || record.PartName || "N/A"
              } đã được duyệt cấp phát cho thiết bị ${
            ...
            console.log(
              "✅ Sent replacement approved notification via SignalR"
            );
